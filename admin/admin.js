...
async function load(){ try{await api('session');content=await api('content');showDashboard()}catch(error){if(!error.unauthorized)$('#login-message').textContent=error.message} }
